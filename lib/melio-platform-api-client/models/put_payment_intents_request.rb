...
module MelioPlatformApiClient
  class PutPaymentIntentsRequest
    # This is your bill's amount
    attr_accessor :amount

    # This is the desired payment amount
    ...
    attr_accessor :delivery_preference
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `MelioPlatformApiClient::PutPaymentIntentsRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `MelioPlatformApiClient::PutPaymentIntentsRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'delivery_preference')
        self.delivery_preference = attributes[:'delivery_preference']
      end
    ...
    def hash
      [amount, payment_amount, schedule_date, funding_source_id, delivery_method_id, delivery_preference, invoice_number, vendor_id, due_date, note].hash
    end
